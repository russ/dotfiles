...
local quake_name = "QuakeTerminal"
local quake_command = terminal .. " --name=" .. quake_name
local quake = Quake:new({
    name = quake_name,
    cmd = quake_command,
    client_props = {
...
local function quake_start()
  awful.spawn.easy_async(
    'pgrep -c -f "' .. quake_command .. '"',
    function (stdout, _, _, exitcode)

      if exitcode ~= 0 then
        quake:spawn()
        naughty.notify({title = "Quake Terminal started"})
      end

      local nterminals = tonumber(stdout)
      if nterminals > 1 then
        naughty.notify({
            preset = naughty.config.presets.critical,
            title = "Too Many Quake Terminals",
            text = nterminals .. " terminals have been spawned"
        })
      end
    end
  )
...
keys.globalkeys = gears.table.join(
    ...
    awful.key({ superkey, altkey   }, "c", function ()
      awful.spawn.with_shell("clipboard-history.sh")
    end,
    {description = "clipboard history", group = "awesome"}),

    -- OrgMode Focus
    awful.key({ superkey, altkey }, "n", function ()
      local all_clients = client.get()
      for i, c in pairs(all_clients) do
        -- naughty.notify({text=c.name})
	    -- Matches the naming scheme of YYYY-MM-DD.org - Doom Emacs to match my note scheme
        if string.match(c.name, "[%d-]*.org.*Emacs")  then
          client.focus = c
        end
      end
    end,
    {description = "orgmode focus", group = "awesome"}),
    ...
    awful.key({ superkey }, "d",
      function()
        ...
        awful.spawn.with_shell("~/.config/rofi/scripts/appsmenu.sh")
      end,
    ...
    -- Screenshots
    awful.key( { superkey, ctrlkey, shiftkey }, "4", function() awful.spawn.with_shell("~/bin/screen-to-dropbox.sh") end,
              {description = "take full screenshot", group = "screenshots"}),

    -- Reload Theme
    awful.key({ superkey, ctrlkey, shiftkey }, "r", function() awful.spawn.with_shell("~/bin/refresh_theme") end,
        {description = "reload theme", group = "awesome"}),
    ...
    --           {description = "take full screenshot", group = "screenshots"}),
    -- awful.key( { superkey, shiftkey, ctrlkey }, "s", function() awful.spawn.with_shell("screenshot.sh -s") end,
    ...
    awful.key({ superkey }, "F6", function() awful.spawn.with_shell("mantablockscreen -cc") end,
    ...
    -- Set centerwork layout
    awful.key({ superkey, shiftkey }, "y", function()
        awful.layout.set(lain.layout.centerwork)
    end,
              {description = "set centerwork layout", group = "tag"}),
    -- Start screen
    ...
    -- mpvtube
    awful.key({ superkey }, "y", function() awful.spawn.with_shell("mpv --script-opts=iptv=1 ~/Dropbox/iptv.m3u") end,
              {description = "iptv", group = "launcher"}),
    -- mpvtube song
